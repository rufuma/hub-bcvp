...
class DialogueSystem:
    ...
    def load(self):
        ...
        self.load_scenario("scenario-买汽车票.json")
    ...
    def fill_in_slot(self, template, memory):
        all_slot_names = self.slot_info.keys()
        for slot in all_slot_names:
            if slot in memory:
                template = template.replace(f"{{{slot}}}", memory[slot])
        return template
...
if __name__ == '__main__':
    ...
    memory = {
        "available_nodes": [
            "scenario-买衣服_node1",
            "scenario-看电影_node1",
            "scenario-买汽车票_node1"]}  # 用户状态
    last_response = ""
    exit_commands = ["退出", "结束", "拜拜", "再见"]
    repeat_commands = ["没听清", "再说一遍", "重复", "再说一次"]

    while True:
        query = input("请输入（支持指令：没听清/退出）：")
        ...
        # 2. 处理重听功能
        if query.strip() in repeat_commands:
            if last_response:
                print(f"重复上一轮回复：{last_response}")
            else:
                print("还没有可重复的内容哦！")
            print("===========")
            continue
