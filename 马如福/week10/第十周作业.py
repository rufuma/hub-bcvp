...
class LanguageModel(nn.Module):
    def __init__(self, vocab):
        ...
        self.encoder = BertModel.from_pretrained(r"D:\AI学习\第六周\第六周 语言模型\bert-base-chinese", return_dict=False)
    ...
    def forward(self, x, y=None):
        ...
        causal_mask = torch.tril(torch.ones((sen_len, sen_len))).long()
        attention_mask = padding_mask.unsqueeze(1) * causal_mask.unsqueeze(0)
...
def train(corpus_path, save_weight=True):
    ...
    for epoch in range(epoch_num):
        model.train()
        watch_loss = []
        train_bar = tqdm(range(batches_per_epoch), desc=f"Epoch {epoch+1}/{epoch_num}")
        for _ in train_bar:
