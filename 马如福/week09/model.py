...
class TorchModel(nn.Module):
    def __init__(self, config):
        ...
        self.loss = torch.nn.CrossEntropyLoss(ignore_index=config["ignore_index"])  #loss采用交叉熵损失
    ...
    def forward(self, input_ids, attention_mask, target=None):
        ...
        if target is not None:
            if self.use_crf:
                crf_tags = target.clone()
                crf_tags[crf_tags == self.loss.ignore_index] = 8
                mask = (target != self.loss.ignore_index).bool()
                mask[:, 0] = True
                #aimport pdb;pdb.set_trace()
                return - self.crf_layer(predict, crf_tags, mask=mask, reduction="mean")
            else:
